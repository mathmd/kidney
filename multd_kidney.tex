...
For $k=\pm 1,\dots,\pm N,\pm(3N+1),\dots,\pm(3N+P),K+1,K+2,K+3$, 
the water flow is given by Poisseuille's equation $\xi_k\alpha_ku_k = -\partial p_k/\partial x$, where $\xi_k\alpha_k$ is the hydrolic resistivity with $\xi_k$ constant and $p_k$ is the hydrostatic pressure, and the corresponding solute flows 
...
\begin{gather}
    \xi_k\alpha_k u_k = -\frac{\partial \tilde{p}_k}{\partial x} - \sum_{i=1}^{M'}z_iFc_i^k\frac{\partial \phi_k}{\partial x},\\
    M'=\begin{cases}
        M&\quad \text{if}\quad k=\pm 1,\dots,\pm N,K+1,\dots,K+3,\\
...
\begin{equation}
    \nu_k(p_k - p_0) = \frac{\alpha_k}{\alpha_k^0}-1,\quad k=1,\dots,N,
\end{equation}
